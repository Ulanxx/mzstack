...
// Eval result collector — accumulates test results, writes to ~/.mzstack-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.mzstack', 'test-transcripts');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/mzstack/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with mzstack installed globally, the fallback path
    // ~/.claude/skills/mzstack/browse/dist/browse exists, so we get READY.
...
OVERRIDE: Write contributor logs to ${logsDir}/ instead of ~/.mzstack/contributor-logs/

Now try this browse command (it will fail — there is no binary at this path):
/nonexistent/path/browse goto https://example.com

This is a mzstack issue (the browse binary is missing/misconfigured).
File a contributor report about this issue. Then tell me what you filed.`,
    ...
    expect(logContent).toContain('Hey mzstack team');
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a mzstack skill. The session preamble detected _SESSIONS=4 (the user has 4 mzstack windows open).
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.mzstack/') && !l.includes('qa-reports/'),
    );
...
`);
    ...
    projectDir = path.join(os.homedir(), '.mzstack', 'projects', 'test-project');
  ...
  test('/plan-eng-review writes test-plan artifact to ~/.mzstack/projects/', async () => {
...
describeIfSelected('mzstack-upgrade E2E', ['mzstack-upgrade-happy-path'], () => {
  let upgradeDir: string;
  let remoteDir: string;

  beforeAll(() => {
    upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-upgrade-'));
    remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mzstack-remote-'));
    ...
    // Create mock mzstack install directory (local-git type)
    const mockGstack = path.join(upgradeDir, '.claude', 'skills', 'mzstack');
    ...
    // Copy mzstack-upgrade skill
    fs.mkdirSync(path.join(upgradeDir, 'mzstack-upgrade'), { recursive: true });
    fs.copyFileSync(
      path.join(ROOT, 'mzstack-upgrade', 'SKILL.md'),
      path.join(upgradeDir, 'mzstack-upgrade', 'SKILL.md'),
    );
  ...
  testIfSelected('mzstack-upgrade-happy-path', async () => {
    const mockGstack = path.join(upgradeDir, '.claude', 'skills', 'mzstack');
    const result = await runSkillTest({
      prompt: `Read mzstack-upgrade/SKILL.md for the upgrade workflow.

You are running /mzstack-upgrade standalone. The mzstack installation is at ./.claude/skills/mzstack (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.claude/skills/mzstack — use that exact path.`,
      workingDirectory: upgradeDir,
      maxTurns: 20,
      timeout: 180_000,
      testName: 'mzstack-upgrade-happy-path',
      runId,
    });

    logCost('/mzstack-upgrade happy path', result);
    ...
    recordE2E('/mzstack-upgrade happy path', 'mzstack-upgrade E2E', result, {
