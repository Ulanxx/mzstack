...
const CODEX_E2E_TOUCHFILES: Record<string, string[]> = {
  ...
  'codex-review-findings':   ['review/**', '.agents/skills/mzstack-review/**', 'codex/**', 'test/helpers/codex-session-runner.ts'],
};
...
describeCodex('Codex E2E', () => {
  ...
  testIfSelected('codex-discover-skill', async () => {
    // Install mzstack-review skill to a temp HOME and ask Codex to list skills
    const skillDir = path.join(testWorktree, '.agents', 'skills', 'mzstack-review');
    ...
    expect(
      outputLower.includes('review') || outputLower.includes('mzstack') || outputLower.includes('skill'),
    ).toBe(true);
  }, 120_000);

  // Validates that Codex can invoke the mzstack-review skill, run a diff-based
  ...
  testIfSelected('codex-review-findings', async () => {
    // Install mzstack-review skill and ask Codex to review the worktree
    const skillDir = path.join(testWorktree, '.agents', 'skills', 'mzstack-review');

    const result = await runCodexSkill({
      skillDir,
      prompt: 'Run the mzstack-review skill on this repository. Review the current branch diff and report your findings.',
      timeoutMs: 540_000,
      cwd: testWorktree,
      skillName: 'mzstack-review',
    });
