...
const MZSTACK_DEV_DIR = path.join(os.homedir(), '.mzstack-dev');
const HEARTBEAT_PATH = path.join(MZSTACK_DEV_DIR, 'e2e-live.json'); // heartbeat stays global
const PROJECT_DIR = path.dirname(getProjectEvalDir()); // ~/.mzstack/projects/$SLUG/
...
export async function runSkillTest(options: {
  ...
  if (browseErrors.length > 0 || exitReason !== 'success') {
    try {
      const failureDir = runDir || path.join(workingDirectory, '.mzstack', 'test-transcripts');
