 ...
 * ~/.mzstack/projects/$SLUG/evals/{version}-{branch}-{tier}-{timestamp}.json,
...
const LEGACY_EVAL_DIR = path.join(os.homedir(), '.mzstack-dev', 'evals');

/**
 * Detect project-scoped eval dir via mzstack-slug.
 * Falls back to legacy ~/.mzstack-dev/evals/ if slug detection fails.
 */
export function getProjectEvalDir(): string {
  try {
    // Try repo-local mzstack-slug first, then global install
    const localSlug = spawnSync('bash', ['-c', '.claude/skills/mzstack/bin/mzstack-slug 2>/dev/null || ~/.claude/skills/mzstack/bin/mzstack-slug 2>/dev/null'], {
    ...
    if (output) {
      ...
      if (slugMatch && slugMatch[1]) {
        const dir = path.join(os.homedir(), '.mzstack', 'projects', slugMatch[1], 'evals');
