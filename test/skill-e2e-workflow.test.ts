...
describeIfSelected('Ship workflow E2E', ['ship-local-workflow'], () => {
  ...
  beforeAll(() => {
    shipRemoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mzstack-ship-remote-'));
    shipWorkDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mzstack-ship-work-'));
...
describeIfSelected('mzstack-upgrade E2E', ['mzstack-upgrade-happy-path'], () => {
  ...
  testConcurrentIfSelected('mzstack-upgrade-happy-path', async () => {
    const mockGstack = path.join(upgradeDir, '.claude', 'skills', 'mzstack');
    const result = await runSkillTest({
...
IMPORTANT: The install directory is at ./.claude/skills/mzstack — use that exact path.`,
    ...
    recordE2E(evalCollector, '/mzstack-upgrade happy path', 'mzstack-upgrade E2E', result, {
