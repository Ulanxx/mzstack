# Contributing to mzstack

Thanks for wanting to make mzstack better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

mzstack skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/mzstack/` (your global install). But when you're developing mzstack itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone <repo> && cd mzstack
...
Contributor mode turns mzstack into a self-improving tool. Enable it and Claude Code
will periodically reflect on its mzstack experience — rating it 0-10 at the end of
each major workflow step. When something isn't a 10, it thinks about why and files
a report to `~/.mzstack/contributor-logs/` with what happened, repro steps, and what
would make it better.

```bash
~/.claude/skills/mzstack/bin/mzstack-config set mzstack_contributor true
```

The logs are for **you**. When something bugs you enough to fix, the report is
already written. Fork mzstack, symlink your fork into the project where you hit
the issue, fix it, and open a PR.

### The contributor workflow

1. **Use mzstack normally** — contributor mode reflects and logs issues automatically
2. **Check your logs:** `ls ~/.mzstack/contributor-logs/`
3. **Fork and clone mzstack** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where mzstack annoyed you)
   ln -sfn /path/to/your/mzstack-fork .claude/skills/mzstack
   cd .claude/skills/mzstack && bun install && bun run build
   ```
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using mzstack** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix mzstack while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ mzstack sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on mzstack inside the mzstack repo

When you're editing mzstack skills and want to test them by actually using mzstack
...
```
mzstack/                          <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── mzstack -> ../../         <- symlink back to repo root
│   ├── review -> mzstack/review
│   ├── ship -> mzstack/ship
...
When E2E tests run, they produce machine-readable artifacts in `~/.mzstack-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.mzstack-dev/` for post-mortem debugging and trend analysis.
...
mzstack generates SKILL.md files for two hosts: **Claude** (`.claude/skills/`) and **Codex** (`.agents/skills/`). Every template change needs to be generated for both.
...
| Aspect | Claude | Codex |
|--------|--------|-------|
| Output directory | `{skill}/SKILL.md` | `.agents/skills/mzstack-{skill}/SKILL.md` (generated at setup, gitignored) |
| Frontmatter | Full (name, description, allowed-tools, hooks, version) | Minimal (name + description only) |
| Paths | `~/.claude/skills/mzstack` | `$MZSTACK_ROOT` (`.agents/skills/mzstack` in a repo, otherwise `~/.codex/skills/mzstack`) |
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/mzstack`. `bin/dev-teardown` restores the global one.
...
**This is the recommended way to develop mzstack.** Symlink your mzstack checkout
into the project where you actually use it, so your changes are live while you
do real work:

```bash
# In your core project
ln -sfn /path/to/your/mzstack-checkout .claude/skills/mzstack
cd .claude/skills/mzstack && bun install && bun run build
```

Now every mzstack skill invocation in this project uses your working tree. Edit a
...
```bash
rm .claude/skills/mzstack
```

Claude Code falls back to `~/.claude/skills/mzstack/` automatically.
