...
echo '{"skill":"land-and-deploy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mzstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
```bash
~/.claude/skills/mzstack/bin/mzstack-review-read 2>/dev/null
```
...
```bash
ls -t ~/.mzstack-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
ls -t ~/.mzstack-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
Then run `mzstack-diff-scope` to classify the changes:

```bash
eval $(~/.claude/skills/mzstack/bin/mzstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
```bash
$B snapshot -i -a -o ".mzstack/deploy-reports/post-deploy.png"
```
...
```bash
mkdir -p .mzstack/deploy-reports
```
...
Save report to `.mzstack/deploy-reports/{date}-pr{number}-deploy.md`.
