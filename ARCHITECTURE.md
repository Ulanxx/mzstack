...
This document explains **why** mzstack is built the way it is. For setup and commands, see CLAUDE.md. For contributing, see CONTRIBUTING.md.

## The core idea

mzstack gives Claude Code a persistent browser and a set of opinionated workflow skills. The browser is the hard part — everything else is Markdown.

The key insight: an AI agent interacting with a browser needs **sub-second latency** and **persistent state**. If every command cold-starts a browser, you're waiting 3-5 seconds per tool call. If the browser dies between commands, you lose cookies, tabs, and login sessions. So mzstack runs a long-lived Chromium daemon that the CLI talks to over localhost HTTP.
...
1. **Compiled binaries.** `bun build --compile` produces a single ~58MB executable. No `node_modules` at runtime, no `npx`, no PATH configuration. The binary just runs. This matters because mzstack installs into `~/.claude/skills/` where users don't expect to manage a Node.js project.
...
The server writes `.mzstack/browse.json` (atomic write via tmp + rename, mode 0o600):
...
Cookies are the most sensitive data mzstack handles. The design:

1. **Keychain access requires user approval.** First cookie import per browser triggers a macOS Keychain dialog. The user must click "Allow" or "Always Allow." mzstack never silently accesses credentials.
...
3. **Database is read-only.** mzstack copies the Chromium cookie DB to a temp file (to avoid SQLite lock conflicts with the running browser) and opens it read-only. It never modifies your real browser's cookie database.
...
```
Browser events → CircularBuffer (in-memory) → Async flush to .mzstack/*.log
```
...
1. **Update check** — calls `mzstack-update-check`, reports if an upgrade is available.
2. **Session tracking** — touches `~/.mzstack/sessions/$PPID` and counts active sessions (files modified in the last 2 hours). When 3+ sessions are running, all skills enter "ELI16 mode" — every question re-grounds the user on context because they're juggling windows.
3. **Contributor mode** — reads `mzstack_contributor` from config. When true, the agent files casual field reports to `~/.mzstack/contributor-logs/` when mzstack itself misbehaves.
...
```bash
jq '.tests[] | select(.exit_reason == "timeout") | .last_tool_call' ~/.mzstack-dev/evals/_partial-e2e.json
```
...
`eval:compare` diffs two eval runs. `eval:summary` aggregates stats across all runs in `~/.mzstack-dev/evals/`.
