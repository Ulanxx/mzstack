...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/mzstack-cli-');
    const execPath = path.join(root, '.claude/skills/mzstack/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/mzstack/browse/src/server.ts');
