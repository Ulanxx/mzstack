/**
 * Tests for bin/mzstack-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * MZSTACK_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'mzstack-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'mzstack-config-test-'));
});
...
describe('mzstack-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { MZSTACK_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
