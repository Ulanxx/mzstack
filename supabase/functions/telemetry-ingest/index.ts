// mzstack telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/mzstack-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.mzstack_version || !event.os || !event.outcome) {
      ...
      rows.push({
        ...
        mzstack_version: String(event.mzstack_version).slice(0, 20),
