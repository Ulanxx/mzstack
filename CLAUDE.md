...
bun run eval:list    # list all eval runs from ~/.mzstack-dev/evals/
...
--verbose`). Results are persisted to `~/.mzstack-dev/evals/` with auto-comparison
...
├── bin/             # CLI utilities (mzstack-repo-mode, mzstack-slug, mzstack-config, etc.)
...
2. **If missing, AskUserQuestion** — let the user tell you or let mzstack search the repo
3. **Persist the answer to CLAUDE.md** so we never have to ask again

This applies to test commands, eval commands, deploy commands, and any other
project-specific behavior. The project owns its config; mzstack reads it.
...
When developing mzstack, `.claude/skills/mzstack` may be a symlink back to this
working directory (gitignored). This means skill changes are **live immediately** —
great for rapid iteration, risky during big refactors where half-written skills
could break other Claude Code sessions using mzstack concurrently.

**Check once per session:** Run `ls -la .claude/skills/mzstack` to see if it's a
symlink or a real copy. If it's a symlink to your working directory, be aware that:
- Template changes + `bun run gen:skill-docs` immediately affect all mzstack invocations
- Breaking changes to SKILL.md.tmpl files can break concurrent mzstack sessions
- During large refactors, remove the symlink (`rm .claude/skills/mzstack`) so the
  global install at `~/.claude/skills/mzstack/` is used instead

**For plan reviews:** When reviewing plans that modify skill templates or the
gen-skill-docs pipeline, consider whether the changes should be tested in isolation
before going live (especially if the user is actively using mzstack in other windows).
...
When estimating or discussing effort, always show both human-team and CC+mzstack time:
...
Contributors can store long-range vision docs and design documents in `~/.mzstack-dev/plans/`.
...
1. Push your branch
2. Fetch and reset in the skill directory: `cd ~/.claude/skills/mzstack && git fetch origin && git reset --hard origin/main`
