...
echo '{"skill":"canary","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mzstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
```bash
eval "$(~/.claude/skills/mzstack/bin/mzstack-slug 2>/dev/null || echo "SLUG=unknown")"
mkdir -p .mzstack/canary-reports
mkdir -p .mzstack/canary-reports/baselines
mkdir -p .mzstack/canary-reports/screenshots
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".mzstack/canary-reports/baselines/<page-name>.png"
$B console --errors
$B perf
$B text
```
...
Save the baseline manifest to `.mzstack/canary-reports/baseline.json`:
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".mzstack/canary-reports/screenshots/pre-<page-name>.png"
$B console --errors
$B perf
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".mzstack/canary-reports/screenshots/<page-name>-<check-number>.png"
$B console --errors
$B perf
```
...
Screenshots:   .mzstack/canary-reports/screenshots/

VERDICT: [DEPLOY IS HEALTHY / DEPLOY HAS ISSUES — details above]
```

Save report to `.mzstack/canary-reports/{date}-canary.md` and `.mzstack/canary-reports/{date}-canary.json`.

Log the result for the review dashboard:

```bash
eval "$(~/.claude/skills/mzstack/bin/mzstack-slug 2>/dev/null)"
mkdir -p ~/.mzstack/projects/$SLUG
```
