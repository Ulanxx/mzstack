...
echo '{"skill":"document-release","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mzstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.mzstack/*" -not -path "./.context/*" | sort
```
...
(adapt to whatever project you're in — these are not mzstack-specific):
...
```bash
gh pr view --json body -q .body > /tmp/mzstack-pr-body-$$.md
```
...
```bash
gh pr edit --body-file /tmp/mzstack-pr-body-$$.md
```

5. Clean up the tempfile:

```bash
rm -f /tmp/mzstack-pr-body-$$.md
```
