...
echo '{"skill":"benchmark","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mzstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
mkdir -p .mzstack/benchmark-reports
mkdir -p .mzstack/benchmark-reports/baselines
```
...
Write to `.mzstack/benchmark-reports/baselines/baseline.json`.
...
Write to `.mzstack/benchmark-reports/{date}-benchmark.md` and `.mzstack/benchmark-reports/{date}-benchmark.json`.
