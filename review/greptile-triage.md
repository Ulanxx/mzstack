...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/mzstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.mzstack/projects/$REMOTE_SLUG/greptile-history.md"
```
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/mzstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.mzstack/projects/$REMOTE_SLUG"
mkdir -p ~/.mzstack
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.mzstack/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.mzstack/greptile-history.md` (global aggregate)
