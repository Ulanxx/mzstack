...
/** mzstack's 10 AI slop anti-patterns — shared between DESIGN_METHODOLOGY and DESIGN_HARD_RULES */
...
MZSTACK_ROOT="$HOME/.codex/skills/mzstack"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.agents/skills/mzstack" ] && GSTACK_ROOT="$_ROOT/.agents/skills/mzstack"
...
\`\`\`bash
${runtimeRoot}_UPD=$(${ctx.paths.binDir}/mzstack-update-check 2>/dev/null || ${ctx.paths.localSkillRoot}/bin/mzstack-update-check 2>/dev/null || true)
...
_CONTRIB=$(${ctx.paths.binDir}/mzstack-config get mzstack_contributor 2>/dev/null || true)
_PROACTIVE=$(${ctx.paths.binDir}/mzstack-config get proactive 2>/dev/null || echo "true")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
echo "PROACTIVE: $_PROACTIVE"
source <(${ctx.paths.binDir}/mzstack-repo-mode 2>/dev/null) || true
REPO_MODE=\${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_LAKE_SEEN=$([ -f ~/.mzstack/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$(${ctx.paths.binDir}/mzstack-config get telemetry 2>/dev/null || true)
...
echo '{"skill":"${ctx.skillName}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mzstack/analytics/skill-usage.jsonl 2>/dev/null || true
# zsh-compatible: use find instead of glob to avoid NOMATCH error
for _PF in $(find ~/.mzstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do [ -f "$_PF" ] && ${ctx.paths.binDir}/mzstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true; break; done
\`\`\``;
}

function generateUpgradeCheck(ctx: TemplateContext): string {
  return `If \`PROACTIVE\` is \`"false"\`, do not proactively suggest mzstack skills — only invoke
them when the user explicitly asks. The user opted out of proactive suggestions.

If output shows \`UPGRADE_AVAILABLE <old> <new>\`: read \`${ctx.paths.skillRoot}/mzstack-upgrade/SKILL.md\` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If \`JUST_UPGRADED <from> <to>\`: tell user "Running mzstack v{to} (just updated!)" and continue.`;
}
...
If A: run \`${ctx.paths.binDir}/mzstack-config set telemetry community\`
...
If B→A: run \`${ctx.paths.binDir}/mzstack-config set telemetry anonymous\`
If B→B: run \`${ctx.paths.binDir}/mzstack-config set telemetry off\`
...
\`\`\`bash
source <(${ctx.paths.binDir}/mzstack-diff-scope <base> 2>/dev/null)
\`\`\`
...
\`\`\`bash
${ctx.paths.binDir}/mzstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
\`\`\`
...
\`\`\`bash
mkdir -p ~/.mzstack/analytics
echo '{"skill":"${_ctx.skillName}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.mzstack/analytics/spec-review.jsonl 2>/dev/null || true
\`\`\`
...
\`~/.claude/skills/mzstack/${first}/SKILL.md\`
...
**Log the result:**
\`\`\`bash
${ctx.paths.binDir}/mzstack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
Replace STATUS with "clean" or "issues_found", SOURCE with "codex+subagent", "codex-only", "subagent-only", or "unavailable".`;
}

// ─── Design Hard Rules (OpenAI framework + mzstack slop blacklist) ───
...
function generateSlugEval(ctx: TemplateContext): string {
  return `eval "$(${ctx.paths.binDir}/mzstack-slug 2>/dev/null)"`;
}

function generateSlugSetup(ctx: TemplateContext): string {
  return `eval "$(${ctx.paths.binDir}/mzstack-slug 2>/dev/null)" && mkdir -p ~/.mzstack/projects/$SLUG`;
}
...
function codexSkillName(skillDir: string): string {
  if (skillDir === '.' || skillDir === '') return 'mzstack';
  // Don't double-prefix: mzstack-upgrade → mzstack-upgrade (not mzstack-mzstack-upgrade)
  if (skillDir.startsWith('mzstack-')) return skillDir;
  return `mzstack-${skillDir}`;
}
...
function processTemplate(tmplPath: string, host: Host = 'claude'): { outputPath: string; content: string } {
  ...
  if (host === 'codex') {
    ...
    content = content.replace(/~\/\.claude\/skills\/mzstack/g, ctx.paths.skillRoot);
    content = content.replace(/\.claude\/skills\/mzstack/g, ctx.paths.localSkillRoot);
    content = content.replace(/\.claude\/skills\/review/g, '.agents/skills/mzstack/review');
