#!/usr/bin/env bun
/**
 * analytics — CLI for viewing mzstack skill usage statistics.
 *
 * Reads ~/.mzstack/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.mzstack', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('mzstack skill usage analytics');
