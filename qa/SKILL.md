...
echo '{"skill":"qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.mzstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
| Mode | full | `--regression .mzstack/qa-reports/baseline.json` |
...
```bash
mkdir -p .mzstack/qa-reports/screenshots
```
...
- Include full attribution comment:
  ...
  // Report: .mzstack/qa-reports/qa-report-{domain}-{date}.md
  ```
